...
def load_fake_recogna() -> pd.DataFrame:
    ...
    for _, r in df.iterrows():
        ...
        rows.append(_row(body, m.get(r["Classe"], str(r["Classe"])),
                         "FakeRecogna",
                         title=r.get("Titulo", ""),
                         url=r.get("URL", "")))
    return pd.DataFrame(rows)
...
def load_faketrue_br() -> pd.DataFrame:
    ...
    rows = []
    for _, r in df.iterrows():
        rows.append(_row(r.get("fake", ""), "False", "FakeTrue.Br",
                         title=r.get("title_fake", ""), url=r.get("link_f", "")))
        rows.append(_row(r.get("true", ""), "True", "FakeTrue.Br",
                         url=r.get("link_t", "")))
    return pd.DataFrame(rows)
...
def load_factck_br() -> pd.DataFrame:
    ...
    for _, r in df.iterrows():
        ...
        rows.append(_row(body, "False" if rating <= 2 else "True",
                         "factck-br",
...
def load_bilstm() -> pd.DataFrame:
    df = pd.read_parquet(
        DB / "portuguese-fake-news-classifier-bilstm-combined"
           / "corpus" / "corpus_test_df.parquet"
    )
    def lbl(v):
        if isinstance(v, bool):
            return "True" if v else "False"
        return str(v).strip()
    rows = [_row(r["text"], lbl(r["label"]),
...
def main():
    ...
    ap.add_argument("--out", default=str(DATA / "corpus_next.v1.nest"),
                    help="output .nest path (default: data/corpus_next.v1.nest)")
    ap.add_argument("--cache", default=str(DB / "corpus-next" / "embed_cache.sqlite"),
    ...
    print(f"   raw rows={n_before:,}  after-filter={n_after_filter:,}  after-dedup={n_after_dedup:,}")
    ...
    def embedder(specs):
        embs = model.encode([s.canonical_text for s in specs],
    ...
    for _, r in combined.iterrows():
        ...
        specs.append(ChunkSpec(canonical_text=r["text"],
                               source_uri=f"corpus-next://{r['source']}/{h}",
                               byte_start=0, byte_end=len(ub)))
    ...
    cfg = BuildConfig(
        ...
        title="corpus-next",
        version="v0.1.0",
        description="Unified PT-BR fake-news corpus combining 7 public datasets, deduplicated by SHA-256 of text.",
        license="mixed (per-source)",
        reproducible=True,
    )
    pipe = Pipeline(cfg, embedder=embedder, scratch_db=args.cache)
    pipe.add_many(specs)
    pipe.emit(provenance={
